...
class _DayDetailsPageState extends State<DayDetailsPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadItems({bool forceRefresh = false}) async {
    ...
    try {
      ...
      print('📥 Loaded ${items.length} items (offline: ${_cacheService.isOfflineMode}, forceRefresh: $forceRefresh):');
      for (int i = 0; i < items.length; i++) {
        print('  Item $i: ${items[i].title} - startTime: ${items[i].startTime}');
      }
      ...
      for (int i = 0; i < items.length; i++) {
        if (items[i].orderIndex != i) {
          print('⚠️ Order index mismatch: Item "${items[i].title}" has order_index ${items[i].orderIndex} but should be $i');
          needsReorder = true;
        }
      }
      ...
      if (needsReorder) {
        ...
        try {
          ...
          final freshItems = await _itemsService.getItemsByItinerary(_itineraryId!);
          if (mounted) {
            setState(() {
              _items = freshItems;
              _isLoading = false;
            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  Text(
                    AppConstants.addedToDay.tr(args: [widget.dayNumber.toString()]),
  ...
  String _formatDate(DateTime date) {
    final months = ['${AppConstants.jan.tr()}', '${AppConstants.feb.tr()}', '${AppConstants.mar.tr()}', '${AppConstants.apr.tr()}', '${AppConstants.may.tr()}', '${AppConstants.jun.tr()}', '${AppConstants.jul.tr()}', '${AppConstants.aug.tr()}', '${AppConstants.sep.tr()}', '${AppConstants.oct.tr()}', '${AppConstants.nov.tr()}', '${AppConstants.dec.tr()}'];
    final days = ['${AppConstants.monday.tr()}', '${AppConstants.tuesday.tr()}', '${AppConstants.wednesday.tr()}', '${AppConstants.thursday.tr()}', '${AppConstants.friday.tr()}', '${AppConstants.saturday.tr()}', '${AppConstants.sunday.tr()}'];
  ...
  Future<void> _downloadItinerary() async {
    ...
    try {
      ...
      final tripTitle = widget.tripCity ?? widget.tripCountry ?? widget.tripTitle;
              ...
              return pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    if (index > 0 && item.travelTimeFromPreviousText != null) ...[
                      pw.Container(
      ...
      await Printing.sharePdf(
        ...
        filename: '${'trip_details.pdf.filename_itinerary'.tr()}_${tripTitle.replaceAll(' ', '_')}_${'trip_details.pdf.filename_day'.tr()}${_currentDayNumber}.pdf',
      );
  ...
  Future<void> _editItemTransport(ItineraryItem item, int index) async {
    ...
    if (result != null && _itineraryId != null) {
      try {
        ...
        final freshItems =
        await _itemsService.getItemsByItinerary(_itineraryId!);
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () async {
                        final added = await _favoritesService.addFavoriteSafe(item.place!.id);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (_isOnline)
            IconButton(
              icon: Icon(Icons.map, color: isDark ? AppColors.textPrimaryDark : AppColors.textPrimaryLight),
              onPressed: () async {
          ...
          if (_isOnline)
            IconButton(
              icon: Icon(Icons.bookmark_border, color: isDark ? AppColors.textPrimaryDark : AppColors.textPrimaryLight),
              onPressed: _showAddToFavoritesSheet,
            ),
        ],
      ),
      floatingActionButton: _effectiveReadOnly ? null : FloatingActionButton(
        onPressed: () {
          showModalBottomSheet(
            context: context,
            isScrollControlled: true,
        ...
        children: [
          ...
          Expanded(
            child: _isLoading
              ...
              itemCount: _effectiveReadOnly ? _items.length : _items.length + 1,
              buildDefaultDragHandles: false,
              onReorder: _effectiveReadOnly ? (_, __) {} : (oldIndex, newIndex) {
              ...
              itemBuilder: (context, index) {
                ...
                return Dismissible(
                  ...
                  direction: _effectiveReadOnly ? DismissDirection.none : DismissDirection.horizontal,
                  confirmDismiss: _effectiveReadOnly ? null : (direction) async {
                    // Show dialog to choose target day
                    final targetDay = await _showMoveToDayDialog(item);
                    if (targetDay != null) {
                  ...
                  child: ReorderableDelayedDragStartListener(
                    ...
                    openingHours: item.place?.openingHours?.getTodayHours(),
                    ...
                    distanceFromPreviousText: item.distanceFromPreviousText,
                    travelTimeFromPreviousText: item.travelTimeFromPreviousText,
...
class _ActivityCardState extends State<_ActivityCard> {
  ...
  String _getOpeningHoursText(String? openingHours) {
    ...
    if (openingHours.trim().toLowerCase() == 'aberto 24 horas' || openingHours.trim().toLowerCase() == 'open 24 hours') {
    ...
    final updatedTransportMode = await Navigator.push<String?>(
      ...
      MaterialPageRoute(
        builder: (context) => NavigationPage(
          ...
          itineraryItemId: currentItem.id, // Pass item ID to save transport changes
        ),
      ),
    );

    // If transport mode was updated, refresh the activity card
    if (updatedTransportMode != null && updatedTransportMode != widget.transportMode) {
            ...
            children: [
              ...
              Stack(
                children: [
                  ...
                  // Número com cor baseada no tipo de transporte
                  Positioned(
                    top: 16,
                    left: 16,
                    child: Container(
                  ...
                  // Botão de delete e ícone de categoria no canto superior direito
                  Positioned(
                    top: 16,
                    right: 16,
                    child: Row(
                  ...
                  // Título e horário sobre a imagem
                  Positioned(
                    bottom: 16,
                    left: 16,
                    right: 16,
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            // Horário de início (sempre mostrar se existir)
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        // Show travel mode and time from previous point (from backend)
                        if (!widget.isStartingPoint && widget.travelTimeFromPreviousText != null) ...[
                          GestureDetector(
                        ...
                        const Spacer(),
                        // Botão Get Directions (não mostrar no primeiro item ou quando offline)
                        if (!widget.isStartingPoint && !widget.isReadOnly)
                          GestureDetector(
...
class _TimeEditDialogState extends State<_TimeEditDialog> {
        ...
        return Theme(
          data: Theme.of(context).copyWith(
            timePickerTheme: TimePickerThemeData(
              ...
              hourMinuteTextColor: widget.isDark ? AppColors.textPrimaryDark : AppColors.textPrimaryLight,
              dialHandColor: AppColors.primary,
              dialBackgroundColor: widget.isDark ? AppColors.grey800 : AppColors.grey200,
            ),
          ),
          child: child!,
        );
  ...
  void _changeDuration(int minutes) {
    setState(() {
      _durationMinutes = (_durationMinutes + minutes).clamp(15, 480); // Min 15min, Max 8h
    });
  }
...
class _EditTransportDialogState extends State<_EditTransportDialog> {
  ...
  String _getEstimatedTime(String mode) {
    ...
    if (mode == widget.currentTransportMode && widget.currentTravelTime != null) {
                    ...
                    child: Material(
                      ...
                      child: InkWell(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: isSelected
                                ? (widget.isDark ? AppColors.primary.withOpacity(0.2) : const Color(0xFFE8F5E9))
                                : Colors.transparent,
                            borderRadius: BorderRadius.circular(8),
                          ),
                          child: Row(
